...
All filesystem paths must be canonicalized before policy checks. Symlink traversal must not escape allowed roots.
...
The daemon must never expose arbitrary shell execution. Commands are executed by absolute path with explicit argument validation.
...
Network tools must enforce host and port allowlists and response size limits.
...
Each event includes timestamp, request ID, session ID when available, caller UID, caller PID, method, risk, decision, parameter summary, error kind when present, and duration when relevant. Sensitive values are summarized or hashed rather than logged in full.

## Testing Strategy

Testing focuses on safety boundaries before feature breadth.

- Policy unit tests cover default denial, capability flags, path allowlists, symlink escape attempts, command allowlists, host allowlists, and resource limits.
- Approval tests cover no-execution-before-approval, approval success, expiry failure, scope checks, and request hash mismatch.
- RPC tests cover valid requests, invalid JSON-RPC, unknown methods, and invalid parameters.
- Tool tests cover read-only system observation, bounded filesystem operations, process timeout/output truncation, and network target denial.
- Integration tests start `anythingd`, send requests through `anythingctl`, approve a pending request, execute it, and verify audit logs.
